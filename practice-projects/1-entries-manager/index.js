...
const entriesManager = (entries, command, key, value) => {
  ...
  //  alert the user and exit early if it is not
  const supportedCommands = ['all', 'read', 'write', 'delete'];
  ...
  const newEntriesString = JSON.stringify(entries, null, 4);
  ...
  fs.writeFile(ENTRIES_PATH, newEntriesString, 'utf-8', writeFileCallback);
};
...
const readFileCb = (err, entriesString) => {
  ...
  entriesManager(parsedEntries, process.argv[2], process.argv[3], process.argv[4]);
};
